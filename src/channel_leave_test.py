...
def initialisation():
    ...
    channel_addowner(Benjamin['token'],channel_id2, Ross['u_id'])
...
def test_multiple_owners():
    ...
    assert channel_leave(Benjamin['token'], channel_id2) == {}
def test_only_owner_alone():
    ...
    assert channel_leave(Benjamin['token'], channel_id3) == {}

def test_is_member():
    _, _, Alex, channel_id0, _, _, _ = initialisation()
    # Person leaving is just a member
    assert channel_leave(Alex['token'], channel_id0) == {}
